...
### 1. 只有稳定 reconciliation snapshot 可以恢复 `SYNCED`

`positions + openOrders + executions` 分别读到不等于同一个原子时点。Adapter 必须
实现明确的 completion/watermark 屏障，并在 `BrokerSnapshot.is_stable=True` 时承担证明责任。
不稳定 snapshot 仅供诊断，不能 terminalize 旧单、不能发送 replacement、不能恢复交易。

### 2. HALT acknowledgement 是精确的 durable CAS

确认动作必须携带当前最新 HALT cause 的 sequence/token。旧页面、旧 token 或新的 HALT
cause 已经出现时，确认必须失败。确认只说明原因已由指定操作员处理，不会让当前进程
恢复 NORMAL；操作员仍需停止进程、人工确认账户、重启并完成 reconciliation。
