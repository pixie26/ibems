...
VHD 上先跑 `wal_corruption` 再跑 `disk_full`（`disk_full` 会吃掉整个卷、只在 `finally` 归还，所以必须
最后跑），且任一 drill 失败不再中断其余 drill——否则一个平台问题会连带把另一个 drill 的结果一起吞掉。

run 31574366903 的直接观测:真实 `ntfs.sys` 上 WAL recovery **静默丢弃 26 条已提交事件**（3653 → 3627），
全部位于 witness `seq=1938` 之上，因此引擎启动是正确的；强制从 seq 1938 截断后 exit `15`、fence
`RAISED`。这是本平台第一次在 Windows 内核文件系统上直接看到"`commit()` 已确认 durable 的事务被无声
