...
| Windows publication 强杀 / ownership | 已完成；post-exit release 边界已加固 | safe drill 4/4：durable replace 回读、两进程单一 owner、holder 强杀后 successor 取锁、publication 中途强杀后仍为完整 JSON generation。2026-08-20 exact-head CI #97 另直接观察到 `Popen.wait()` 返回后两条测试仍短暂收到 byte-range lock refusal；失败证据保留。known-exit 测试/drill 现仅对该边界做 2 秒/10ms 有界重试并记录 retry/elapsed；生产 `ProcessLock.acquire()` 仍为非阻塞 fail-loudly | 不允许第二个 live writer 排队；只有调用方已证明旧 holder 退出的 evidence/test boundary 可重试。local full suite `511 passed, 1 skipped`；exact-fix CI 待本提交交付后验证 |
