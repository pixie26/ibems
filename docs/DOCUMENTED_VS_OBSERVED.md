...
| RTH BidAsk / AllLast / 5s bars | `PENDING DOC REVIEW` | 正式 preflight 120.109 秒：`25665/3168/25`；bounded Recorder 120.360 秒落盘：`15590/2843/25`，市场数据行均为 LIVE | `OBSERVED - RTH BOUNDED PASS`（只是「三路拿到 LIVE 数据」，**不是「Recorder 无损」**） | report SHA `204c979b...14c1b` / `86de4371...43d`；不是 Full-RTH 全日 health。两轮 recorder BidAsk 均比配对 preflight 少约 40%（RTH −39.3%、OVERNIGHT −43.0%）而 AllLast 仅 −10.3%/+23%；订阅参数与计数语义已核对为相同，原因未定，判定实验见 [`GATE_B2_REVIEW_20260810.md`](GATE_B2_REVIEW_20260810.md) §3 |
...
| `1100 / 1101 / 1102` | 1100=Gateway/TWS 丢失 IB server 连接；1101/1102=恢复且分别表示 market data lost/maintained | Gateway 进程存活的外网阻断轮收到 1100，清理规则后收到 1102；没有收到 1101；1102 后 server time 和 broker snapshot 均成功 | `PARTIAL - 1100/1102 OBSERVED` | report SHA-256 `f8c88498...c8bc2`；1101 仍未直接观察，不得由 1102 推断。**未观察到 1101 是 probe 设计的必然结果**：该 probe 断网期间不持有任何行情订阅，没有可丢的 market data，1102 是唯一可达结果。确定性配方与后续静默订阅死亡测试见 [`GATE_B2_REVIEW_20260810.md`](GATE_B2_REVIEW_20260810.md) §5 |
