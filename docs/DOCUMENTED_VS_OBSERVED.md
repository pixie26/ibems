...
| RTH BidAsk / AllLast / 5s bars | `PENDING DOC REVIEW` | 正式 preflight 120.109 秒：`25665/3168/25`；bounded Recorder 120.360 秒落盘：`15590/2843/25`，市场数据行均为 LIVE；2026-08-11 handler-count run 120.406 秒：handler/readback 均为 `8972/1707/25` | `OBSERVED - RTH BOUNDED PASS`（只是「三路拿到 LIVE 数据」；新实验另证明该窗口 handler→raw readback 无丢失，**仍不是端到端 Recorder 无损**） | 新 report SHA `0cb4c95b...e02edc`。两组旧顺序窗口 Recorder BidAsk 比先前 preflight 少约 40%，但时间间隔使其不能排除市场漂移。归因 callback 前差异需同步、独立进程 A/B，见 [`GATE_B2_REVIEW_20260810.md`](GATE_B2_REVIEW_20260810.md) §3 |
...
| `1100 / 1101 / 1102` | 1100=Gateway/TWS 丢失 IB server 连接；1101/1102=恢复且分别表示 market data lost/maintained | Gateway 进程存活的外网阻断轮收到 1100，清理规则后收到 1102；没有收到 1101；1102 后 server time 和 broker snapshot 均成功 | `PARTIAL - 1100/1102 OBSERVED` | report SHA-256 `f8c88498...c8bc2`；1101 仍未直接观察，不得由 1102 推断。原 probe 不持有行情订阅，因此没有覆盖“既有订阅丢失并需重订阅”的分支；但官方文档不保证无订阅必为 1102，也不保证固定断网时长加订阅必为 1101。改进实验应持订阅并记录实际 recovery code、staleness 与 resubscribe，见 [`GATE_B2_REVIEW_20260810.md`](GATE_B2_REVIEW_20260810.md) §5 |
