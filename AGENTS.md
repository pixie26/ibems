...
- Before a non-trivial behavior change, reproduce or tightly bound the symptom and identify the governing invariant, canonical owner, structural root cause, correct fix layer, affected consumers, regression proof, and required evidence level.
- Match depth to risk. Mechanical changes stay minimal; state, broker, concurrency, persistence, recovery, or lifecycle changes require architecture and end-to-end consumer analysis before editing.
- Fix the invariant and ownership, not the visible symptom. If a fix immediately needs extra retries, timers, locks, guards, observers, or special cases, stop and re-audit ownership, lifecycle, event flow, and dependency boundaries.
