...
def _repository(tmp_path: Path) -> tuple[Path, str, str, dict[str, str]]:
    ...
    (repo / "docs" / "review.md").write_text("frozen review\n", encoding="utf-8")
...
def _artifact_archive(commit: str, tree: str) -> bytes:
    identity = {
        "schema_version": 1,
        "commit_sha": commit,
        "tree_sha": tree,
    ...
    with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        archive.writestr("checkout-identity.json", json.dumps(identity, sort_keys=True))
    return output.getvalue()


def _material(tmp_path: Path, commit: str, tree: str, hashes: dict[str, str]):
    root = tmp_path / "evidence"
    root.mkdir()
    (root / "v4_health.json").write_bytes(b"{\"health_ok\":true}\n")
    (root / "v3_fail.json").write_bytes(b"{\"health_ok\":false}\n")
    archive = _artifact_archive(commit, tree)
    payload = _manifest(commit, hashes)
    payload["ci_artifacts"] = [
        {
            "run_id": 100,
            "job_name": "verify",
            "artifact_id": 200,
    ...
    def lookup(repository: str, artifact_id: int) -> dict[str, object]:
        assert repository == "owner/repo"
...
def test_populates_from_git_objects_and_streamed_files(tmp_path: Path) -> None:
    ...
    report = b2_evidence_material.verify_materials(
        ...
        github_artifact_lookup=artifact_lookup,
...
def test_repo_evidence_is_read_from_capture_commit_not_dirty_worktree(tmp_path: Path) -> None:
    repo, commit, tree, hashes = _repository(tmp_path)
    payload, roots, artifact_lookup = _material(tmp_path, commit, tree, hashes)
    roots["repo"] = repo
    evidence = payload["evidence"][0]  # type: ignore[index]
    evidence["relative_path"] = "repo/docs/review.md"  # type: ignore[index]
    (repo / "docs" / "review.md").write_text("dirty replacement\n", encoding="utf-8")
    b2_evidence_material.verify_materials(
        payload,
        repo_root=repo,
        controlled_roots=roots,
    ...
    blob = subprocess.run(
        ["git", "-C", str(repo), "show", f"{commit}:docs/review.md"],
        capture_output=True,
        check=True,
    ).stdout
    assert evidence["sha256"] == hashlib.sha256(blob).hexdigest()  # type: ignore[index]
    assert evidence["bytes"] == len(blob)  # type: ignore[index]


def test_artifact_checkout_identity_must_be_exact_candidate(tmp_path: Path) -> None:
    repo, commit, tree, hashes = _repository(tmp_path)
    payload, roots, artifact_lookup = _material(tmp_path, commit, tree, hashes)
    observed = artifact_lookup("owner/repo", 200)
    archive = observed["archive"]
    assert isinstance(archive, bytes)
    members = b2_evidence_material._zip_members(archive)
    identity = json.loads(members["checkout-identity.json"])
    identity["commit_sha"] = "9" * 40
    output = io.BytesIO()
    with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_DEFLATED) as rebuilt:
        rebuilt.writestr("checkout-identity.json", json.dumps(identity, sort_keys=True))
    wrong_archive = output.getvalue()
    payload["ci_artifacts"][0]["archive_sha256"] = (  # type: ignore[index]
        hashlib.sha256(wrong_archive).hexdigest()
    )

    def wrong_lookup(repository: str, artifact_id: int) -> dict[str, object]:
        result = artifact_lookup(repository, artifact_id)
        metadata = dict(result["metadata"])  # type: ignore[arg-type]
        metadata["digest"] = f"sha256:{hashlib.sha256(wrong_archive).hexdigest()}"
        metadata["size_in_bytes"] = len(wrong_archive)
        return {"metadata": metadata, "archive": wrong_archive}

    with pytest.raises(
        b2_evidence_material.B2MaterialVerificationError,
        match="checkout identity commit_sha does not match",
    ):
        b2_evidence_material.verify_materials(
            payload,
            repo_root=repo,
            ...
            github_repository="owner/repo",
            github_lookup=_lookup(commit),
            github_artifact_lookup=wrong_lookup,
            populate_observed=True,
            now=datetime(2026, 8, 21, tzinfo=timezone.utc),
        )


def test_ci_artifact_must_cover_every_observed_job(tmp_path: Path) -> None:
    repo, commit, tree, hashes = _repository(tmp_path)
    payload, roots, artifact_lookup = _material(tmp_path, commit, tree, hashes)

    def extra_job_lookup(repository: str, run_id: int, attempt: int) -> dict[str, object]:
        ...
        observed["jobs_total_count"] = 2
        return observed

    with pytest.raises(
        b2_evidence_material.B2MaterialVerificationError,
        match="cover every observed successful job",
    ):
        b2_evidence_material.verify_materials(
            payload,
            repo_root=repo,
            controlled_roots=roots,
            github_repository="owner/repo",
            github_lookup=extra_job_lookup,
            github_artifact_lookup=artifact_lookup,
            populate_observed=True,
            now=datetime(2026, 8, 21, tzinfo=timezone.utc),
        )
...
def test_bound_ci_evidence_member_must_exist_in_artifact(tmp_path: Path) -> None:
    repo, commit, tree, hashes = _repository(tmp_path)
    payload, roots, artifact_lookup = _material(tmp_path, commit, tree, hashes)
    (roots["evidence"] / "ci_output.json").write_bytes(b"ci output\n")
    payload["evidence"].append(  # type: ignore[union-attr]
        _entry("CI_OUTPUT", "CI_ARTIFACT", "PASS", commit)
    )
    artifact = payload["ci_artifacts"][0]  # type: ignore[index]
    artifact["evidence_id"] = "CI_OUTPUT"  # type: ignore[index]
    artifact["evidence_member_path"] = "ci_output.json"  # type: ignore[index]
    with pytest.raises(
        b2_evidence_material.B2MaterialVerificationError,
        match="missing evidence member",
    ):
        b2_evidence_material.verify_materials(
            payload,
            repo_root=repo,
