...
from . import attestation, b2_frozen, gate, tree_identity

STATE_FILENAME = "STATE.json"

# Compatibility aliases are references to the canonical registry, not copies.
SOURCE_ROOTS = tree_identity.SOURCE_ROOTS
SOURCE_SUFFIXES = tree_identity.SOURCE_SUFFIXES
SOURCE_EXTRA = tree_identity.SOURCE_EXTRA_FILES
CONFIG_GLOBS = tree_identity.CONFIG_GLOBS
LOCK_FILENAME = tree_identity.DEPENDENCY_LOCK_FILE


def source_tree_sha256(root: Path) -> tuple[str, list[str]]:
    identity = tree_identity.derive_from_worktree(root)
    return identity.source_tree_sha256, list(identity.source_files)


def config_tree_sha256(root: Path) -> tuple[str, list[str]]:
    identity = tree_identity.derive_from_worktree(root)
    return identity.config_tree_sha256, list(identity.config_files)
...
def dependency_lock_sha256(root: Path) -> Optional[str]:
    ...
    if not lock.exists():
        return None
    return tree_identity.derive_from_worktree(root).dependency_lock_sha256
...
def tree_state(root: Path) -> dict[str, Any]:
    ...
    """
    return tree_identity.derive_from_worktree(root).as_state()
...
def write_state(root: Path) -> Path:
    ...
    state = {
        ...
        "_comment": (
            ...
            "current-worktree attestation coverage, and B2 read-only evidence identity. "
