...
def derived_gate_status(root: Path) -> dict[str, Any]:
    """Recompute gate status from registry + durable exact-freeze attestation."""
    signed_off_commit = attestation.derive_signed_off_commit(root)
    return gate.as_state(signed_off_commit=signed_off_commit)


def write_state(root: Path) -> Path:
    """Regenerate ``STATE.json``. The only supported way to change it.

    Neither requirement status nor Gate B1 PASS is carried forward from the
    existing file. Requirements come from ``gate.B1_REQUIREMENTS`` and PASS is
    recomputed from the signed exact-freeze attestation. This means a valid PASS
    survives regeneration, while a later source/config/test/dependency change
    automatically makes the old attestation non-derivable.
    """
    state = {
        "schema_version": 2,
        "_comment": (
            "GENERATED by `python -m ib_execution.provenance` -- do not hand-edit. "
            "gate_status is derived from ib_execution.gate.B1_REQUIREMENTS plus "
            "the validated exact-freeze attestation. tests/test_provenance.py "
