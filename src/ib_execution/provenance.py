...
def write_state(root: Path, gate_status: Optional[dict[str, Any]] = None) -> Path:
    ...
    ``gate_status`` is *derived* from ``gate.B1_REQUIREMENTS``, never carried
    forward from the existing file. Carrying it forward is precisely how B1.6
    was added to the registry, to three documents, and still never appeared in
    the file the README calls authoritative.
    """
    state = {
        "schema_version": 2,
        "_comment": (
            "GENERATED by `python -m ib_execution.provenance` -- do not hand-edit. "
            "gate_status is derived from ib_execution.gate.B1_REQUIREMENTS. "
            "tests/test_provenance.py fails when this file is stale."
        ),
        "tree": tree_state(root),
        "gate_status": gate_status if gate_status is not None else gate.as_state(),
    }
    path = root / STATE_FILENAME
...
def stale_fields(root: Path) -> dict[str, tuple[Any, Any]]:
    """Recorded-vs-actual, for the tree hashes *and* the requirement registry.

    Checking only the hashes was the second half of the B1.6 drift: the tree
    matched, so nothing failed, while the gate section quietly described a
    different set of blockers from the one in code.
    """
    ...
    expected_gate = gate.as_state()
    recorded_gate = recorded.get("gate_status", {})
    for key, value in expected_gate.items():
        if key in ("gate_b1", "gate_b2", "trading_adapter", "signed_off_commit"):
            continue          # a sign-off legitimately changes these
